...
class TestComputeMetrics:
    ...
    def test_partial_predictions(self):
        ...
        assert (
            pytest.approx(metrics["token_accuracy"], 0.01) == 4 / 6
        )  # 4 correct out of 6 tokens
...
class TestComputeCrossEntropyLoss:
    ...
    def test_all_padded(self):
        ...
        with patch(
            "torch.nn.functional.cross_entropy", return_value=torch.tensor(float("nan"))
        ):
...
class TestLMLossAndMetricFn:
    ...
    def test_loss_and_metric_fn_basic(
        self, mock_pipeline, mock_intervenable_model, mock_batch, mock_model_units_list
    ):
        """Test basic functionality of loss_and_metric_fn."""
        # Mock _prepare_intervenable_inputs to return expected values
        with patch(
            "experiments.residual_stream_experiment._prepare_intervenable_inputs"
        ) as mock_prepare:
            # Configure the mock
...
class TestPatchResidualStream:
    ...
    @pytest.fixture
    def patch_object(
        self, mock_pipeline, mock_causal_model, mock_token_positions, mock_checker
    ):
        ...
        obj.model_units_lists = [[[mock_unit]]]  # type: ignore
    ...
    def test_extract_metadata_map(self, patch_object):
        ...
        results = {
            "dataset": {
                "dataset1": {
                    "model_unit": {
                        "unit1": {"metadata": {"layer": 0, "position": "pos1"}},
                        "unit2": {"metadata": {"layer": 1, "position": "pos2"}},
                    }
                }
            }
        }
    ...
    def test_create_heatmap(self, patch_object):
        ...
        with (
            ...
            patch_object._create_heatmap(
                score_matrix, layers, positions, title, save_path="/fake/path/test.png"
            )
    ...
    def test_plot_average_heatmap(self, patch_object):
        ...
        # Call the method
        patch_object._plot_average_heatmap(
            results, layers, positions, target_variables_str
        )
    ...
    def test_plot_individual_heatmaps(self, patch_object):
        ...
        # Call the method
        patch_object._plot_individual_heatmaps(
            results, layers, positions, target_variables_str
        )
    ...
    def test_plot_heatmaps_integration(self, patch_object):
        ...
        patch_object._extract_metadata_map = MagicMock(
            return_value={"unit1": {"layer": 0, "position": "pos1"}}
        )
        ...
        # Test with averaging
        patch_object.plot_heatmaps(
            results, ["var1", "var2"], average_counterfactuals=True
        )
