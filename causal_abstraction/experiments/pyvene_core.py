...
def _prepare_intervenable_model(
    ...
    intervenable_model.set_device(pipeline.model.device)  # type: ignore
...
def _prepare_intervenable_inputs(pipeline, batch, model_units_list):
    ...
    if (
        ...
        and model_units_list[0][0].component.unit != "h.pos"
    ):
        pad_token_id = pipeline.tokenizer.pad_token_id
        # Update base_indices to account for the padding
        base_indices = [
            [
                [j + (base == pad_token_id).sum().item() for j in index]
                for base, index in zip(batched_base["input_ids"], indices)
            ]
        ...
        counterfactual_indices = [
            [
                [j + (counterfactual == pad_token_id).sum().item() for j in index]
                for counterfactual, index in zip(
                    batched_counterfactual["input_ids"], indices
                )
            ]
            for indices, batched_counterfactual in zip(
                counterfactual_indices, extended_batched_counterfactuals
            )
        ]
...
def _batched_interchange_intervention(
    pipeline, intervenable_model, batch, model_units_list, output_scores=False
):
    """
...
def _run_interchange_interventions(
    ...
    dataloader = DataLoader(
        counterfactual_dataset,  # type: ignore
    ...
    for batch in tqdm(dataloader, desc="Processing batches", disable=not verbose):
        with torch.no_grad():  # Disable gradient tracking for inference
            ...
            if output_scores:
                ...
                scores_or_sequences = torch.stack(
                    [score.clone().detach().to("cpu") for score in scores_or_sequences],
                    1,
                )
            else:
...
def _train_intervention(
    ...
    temperature_schedule: torch.Tensor | None = None  # type: ignore
    if intervention_type == "mask":
        ...
        temperature_schedule = temperature_schedule.to(pipeline.model.dtype).to(  # type: ignore
            pipeline.model.device  # type: ignore
        )
    ...
    for epoch in train_iterator:
        epoch_iterator = tqdm(
            dataloader, desc=f"Epoch: {epoch}", position=0, leave=True
        )
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            for k, v in batch.items():
                if v is not None and isinstance(v, torch.Tensor):
                    batch[k] = v.to(pipeline.model.device)  # type: ignore
    ...
    if intervention_type == "mask":
        ...
        for kv, model_unit in zip(
            ...
            tb_writer.add_text(
                "Selected features", f"Number Selected features: {len(indices)}"
            )
