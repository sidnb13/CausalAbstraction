    ...
    try:
        ...
        for k in batched_base:
            if isinstance(batched_base[k], torch.Tensor):
                batched_base[k] = torch.cat(
                    [batched_base[k], batched_inv_label[k]], dim=-1
                )
        ...
        eval_metrics = compute_metrics(
            pred_ids, labels, pipeline.tokenizer.pad_token_id
        )
        loss = compute_cross_entropy_loss(
            logits, labels, pipeline.tokenizer.pad_token_id
        )
...
def compute_metrics(predicted_token_ids, eval_labels, pad_token_id):
    ...
    try:
        ...
        token_accuracy = (
            correct_tokens.sum().float() / mask.sum()
            if mask.sum() > 0
            else torch.tensor(1.0)
        )

        # Calculate sequence-level accuracy (sequence correct if all non-pad tokens correct)
        sequence_correct = torch.stack(
            [torch.all(correct_tokens[i, mask[i]]) for i in range(eval_labels.shape[0])]
        )
        sequence_accuracy = (
            sequence_correct.float().mean()
...
class PatchResidualStream(InterventionExperiment):
                ...
                for model_units in model_units_list:  # type: ignore
                    for unit in model_units:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to load SAE for layer {layer}: {str(e)}"
                            )
    ...
    def plot_heatmaps(
        ...
        if average_counterfactuals:
            self._plot_average_heatmap(
                results, layers, positions, target_variables_str, save_path
            )
        else:
            self._plot_individual_heatmaps(
                results, layers, positions, target_variables_str, save_path
            )
    ...
    def _extract_metadata_map(self, results: Dict) -> Dict:
        ...
        for unit_str, unit_data in results["dataset"][first_dataset][
            "model_unit"
        ].items():
        ...
        for dataset_name in results["dataset"]:
            ...
            for i, layer in enumerate(layers):
                for j, pos in enumerate(positions):
                    for unit_str, unit_data in results["dataset"][dataset_name][
                        "model_unit"
                    ].items():
                        if (
                            "metadata" in unit_data
                            and target_variables_str in unit_data
                        ):
                            if "average_score" in unit_data[target_variables_str]:
                                metadata = unit_data["metadata"]
                                if (
                                    metadata.get("layer") == layer
                                    and metadata.get("position") == pos
                                ):
                                    temp_matrix[i, j] = unit_data[target_variables_str][
                                        "average_score"
                                    ]
                                    valid_entries = True
        ...
        if dataset_count > 0:
            ...
            self._create_heatmap(
                ...
                save_path=os.path.join(
                    save_path, f"heatmap_average_{results['task_name']}.png"
                )
                if save_path
                else None,
            )
    ...
    def _plot_individual_heatmaps(
        ...
        for dataset_name in dataset_names:
                        ...
                        if (
                            ...
                            if "average_score" in unit_data[target_variables_str]:
                                ...
                                if (
                                    ...
                                    score_matrix[i, j] = unit_data[
                                        target_variables_str
                                    ]["average_score"]
                                    valid_entries = True
            ...
            if valid_entries:
                ...
                safe_dataset_name = (
                    dataset_name.replace(" ", "_").replace("/", "_").replace("\\", "_")
                )
                ...
                self._create_heatmap(
                    ...
                    save_path=os.path.join(
                        save_path,
                        f"heatmap_{safe_dataset_name}_{results['task_name']}.png",
                    )
                    if save_path
                    else None,
                )
