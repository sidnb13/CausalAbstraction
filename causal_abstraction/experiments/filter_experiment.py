...
class FilterExperiment:
    ...
    def filter(
        ...
        for dataset_name, counterfactual_dataset in counterfactual_datasets.items():
            ...
            for b_i in tqdm(
                range(0, len(dataset["input"]), batch_size),
                ...
                orig_valid = self._validate_original_inputs(
                    dataset, orig_inputs, b_i, batch_size
                )

                # Process counterfactual inputs
                all_cf_inputs = dataset["counterfactual_inputs"][b_i : b_i + batch_size]
                cf_valid = self._validate_counterfactual_inputs(
                    dataset, all_cf_inputs, b_i, batch_size
                )
            ...
            if "input" not in filtered_data or not filtered_data["input"]:
                if verbose:
                    print(
                        f"Dataset '{dataset_name}' has no valid examples after filtering."
                    )
                continue
            ...
            if verbose:
                ...
                print(
                    f"Dataset '{dataset_name}': kept {dataset_kept}/{dataset_original} examples "
                    f"({keep_rate:.1f}%)"
                )
    ...
    def _validate_original_inputs(
        self, dataset: Any, orig_inputs: List[Any], batch_idx: int, batch_size: int
    ) -> List[bool]:
        """
        ...
        orig_expected = [
            self.causal_model.run_forward(x)["raw_output"] for x in orig_inputs
        ]

        # Check validity
        return [self.checker(pred, exp) for pred, exp in zip(orig_preds, orig_expected)]
    ...
    def _validate_counterfactual_inputs(
        ...
        cf_flattened_preds = self.pipeline.dump(
            self.pipeline.generate(cf_flattened_inputs)
        )
        ...
        return [
            all(
                ...
                for pred, exp in zip(cf_preds[i], cf_expected[i])
            )
