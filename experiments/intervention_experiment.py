...
class InterventionExperiment:
    ...
    def perform_interventions(
        ...
        results = {
            ...
            "dataset": {
                dataset_name: {
                    "model_unit": {
                        str(units_list): None for units_list in self.model_units_lists
                    }
                }
        ...
        for dataset_name in datasets.keys():
            for model_units_list in self.model_units_lists:
                ...
                if verbose:
                    print(
                        f"Running interventions for {dataset_name} with model units {model_units_list}"
                    )
                ...
                results["dataset"][dataset_name]["model_unit"][
                    str(model_units_list)
                ] = {"raw_outputs": raw_outputs, "metadata": metadata}

                # Process and decode model outputs
                dumped_outputs = []
                for raw_output in raw_outputs:
                    dump_result = self.pipeline.dump(
                        raw_output, is_logits=self.config["output_scores"]
                    )
                ...
                for target_variables in target_variables_list:
                    ...
                    labeled_data = self.causal_model.label_counterfactual_data(
                    ...
                    assert len(labeled_data) == len(raw_outputs), (
                        f"Length mismatch: {len(labeled_data)} vs {len(raw_outputs)}"
                    )

                    # Compute intervention scores
                    scores = []
                    for example, output, raw_output in zip(
                        labeled_data, dumped_outputs, raw_outputs
                    ):
                    ...
                    results["dataset"][dataset_name]["model_unit"][
                        str(model_units_list)
                    ][target_variable_str]["scores"] = scores
                    results["dataset"][dataset_name]["model_unit"][
                        str(model_units_list)
                    ][target_variable_str]["average_score"] = np.mean(scores)
                ...
                if not self.config["output_scores"]:
                    del results["dataset"][dataset_name]["model_unit"][
                        str(model_units_list)
                    ]["raw_outputs"]
        ...
        if save_dir is not None:
            ...
            for dataset_name in results["dataset"].keys():
                for model_unit in results["dataset"][dataset_name][
                    "model_unit"
                ].values():
                    ...
                    for target_variable in model_unit.keys():
                        if (
                            model_unit[target_variable] is not None
                            and "scores" in model_unit[target_variable]
                        ):
        ...
        for model_units_list in self.model_units_lists:
            for model_units in model_units_list:
                for model_unit in model_units:
                    ...
                    if os.path.exists(filename + "_featurizer") and os.path.exists(
                        filename + "_inverse_featurizer"
                    ):
    ...
    def build_SVD_feature_interventions(
        self,
        datasets,
        n_components=None,
        verbose=False,
        ...
        zipped_model_units = [
            list(chain.from_iterable(model_units_list))
        ...
        for i, model_units_list in enumerate(self.model_units_lists):
            for j, model_units in enumerate(model_units_list):
                for k, model_unit in enumerate(model_units):
                    ...
                    if verbose:
                        print(
                            f"SVD explained variance: {[round(float(x), 2) for x in svd.explained_variance_ratio_]}"
                        )

                    model_unit.set_featurizer(
                        SubspaceFeaturizer(
                            rotation_subspace=rotation.T, trainable=False, id="SVD"
                        )
                    )
                    model_unit.set_feature_indices(None)  # Use all components initially

    def train_interventions(
        self, datasets, target_variables, method="DAS", model_dir=None, verbose=False
    ):
        """
        ...
        counterfactual_dataset = []
        for dataset in datasets.values():
            counterfactual_dataset += self.causal_model.label_counterfactual_data(
                dataset, target_variables
            )
        ...
        for idx, model_units_list in enumerate(self.model_units_lists):
            # Extract metadata for verbose logging
            if verbose:
                metadata = (
                    self.metadata_fn(model_units_list)
                    if hasattr(self, "metadata_fn") and self.metadata_fn
                    else {}
                )
                layer = metadata.get("layer", "unknown")
                position = metadata.get("position", "unknown")
                print(f"\n{'=' * 60}")
                print(f"Training intervention {idx + 1}/{len(self.model_units_lists)}")
                print(f"Layer: {layer}, Position: {position}")
                print(f"Method: {method}, Target Variables: {target_variables}")
                print(f"{'=' * 60}")
            ...
            for model_units in model_units_list:
                for model_unit in model_units:
                    if method == "DAS":
                        ...
                        model_unit.set_featurizer(
                            Featurizer(n_features=model_unit.shape[0], id="DBM_OLD")
                        )
                        model_unit.set_feature_indices(None)  # Use all features

            # Train the intervention
            if verbose:
                print(f"Starting training for Layer {layer}, Position {position}...")

            final_stats = _train_intervention(
                self.pipeline,
                model_units_list,
                counterfactual_dataset,
                intervention_type,
                self.config,
                self.loss_and_metric_fn,
            )

            if verbose:
                print(f"Completed training for Layer {layer}, Position {position}")
                if final_stats:
