    ...
    for i, model_units in enumerate(model_units_list):
        for model_unit in model_units:
            config = model_unit.create_intervention_config(
                i, intervention_type, mask_kwargs
            )
...
def _train_intervention(
    ...
    # Extract mask intervention kwargs if available
    mask_kwargs = (
        config.get("mask_intervention_kwargs", None)
        if intervention_type == "mask_new"
        else None
    )
    intervenable_model = _prepare_intervenable_model(
        pipeline,
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if intervention_type == "mask_new":
                ...
                for k, v in intervenable_model.interventions.items():
                    if isinstance(v, tuple):
                        intervenable_model.interventions[k][0].step_temperature(
                            total_steps
                        )
                    else:
                        intervenable_model.interventions[k].step_temperature(
                            total_steps
                        )
    ...
    if intervention_type in ["mask_old", "mask_new"]:
        ...
        for kv, model_unit in zip(
            ...
            mask_binary = v.parameterize().float().cpu()
